...
# CC Studio Server — deploy / update script
# Usage: sudo bash deploy.sh
# All paths can be overridden via environment variables:
#   APP_DIR=/my/app DATA_DIR=/my/data sudo -E bash deploy.sh

CONF_DIR="${CONF_DIR:-/etc/cc-studio}"
...
APP_DIR="${APP_DIR:-/opt/cc-studio-server}"
DATA_DIR="${DATA_DIR:-/var/lib/cc-studio}"
SERVICE_USER="${SERVICE_USER:-cc-studio}"
SERVICE_NAME="${SERVICE_NAME:-cc-studio-server}"

echo "==> CC Studio Server Deploy"
...
ln -sf "$APP_DIR/admin.mjs" /usr/local/bin/cc-studio-admin
...
cp "$SCRIPT_DIR/cc-studio-server.service" /etc/systemd/system/"$SERVICE_NAME".service
