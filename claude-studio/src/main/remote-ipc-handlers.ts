...
async function handleRemoteCommand(fromDeviceId: string, parsed: unknown): Promise<void> {
  ...
  const channelAliases: Record<string, string> = {
    'studio:spawn': 'claude:spawn',
    'studio:send': 'claude:send',
    'studio:kill': 'claude:kill',
    'studio:permission-response': 'claude:permission-response',
    'studio:setPermissionMode': 'claude:setPermissionMode',
  };
  ...
  try {
    ...
    if (internalChannel === 'claude:spawn' && typeof result === 'string') {
      processToDevice.set(result, fromDeviceId);
    }

    // Clean up mapping when a remote kill is called
    if (internalChannel === 'claude:kill' && msg.args?.[0]) {
