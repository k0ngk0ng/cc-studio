...
const STORAGE_KEY = 'codex-studio-settings';

const DEFAULT_PROFILE_ID = 'default';

function createDefaultProfile(): CodexProfile {
  return {
...
function saveSettings(settings: AppSettings) {
  // Fire-and-forget write to ~/.codex-studio/settings.json
...
function mergeWithDefaults(parsed: Record<string, unknown>): AppSettings {
  ...
  const settings: AppSettings = {
    ...
    provider: {
      ...
      profiles: (providerSource.profiles as CodexProfile[]) || defaultSettings.provider.profiles,
  ...
  // Migrate old per-tool permission values to Codex exec modes.
  const modeMap: Record<string, string> = {
    suggest: 'workspaceWrite',
    'auto-edit': 'workspaceWrite',
    'full-auto': 'fullAuto',
  ...
  const validModes = ['readOnly', 'workspaceWrite', 'fullAuto', 'dangerFullAccess'];
  if (!validModes.includes(settings.general.autoApprove)) {
    settings.general.autoApprove = (modeMap[settings.general.autoApprove] || 'workspaceWrite') as PermissionMode;
  }
...
async function loadSettingsFromFile(): Promise<AppSettings> {
  try {
    ...
    try {
      const stored = localStorage.getItem(STORAGE_KEY) || localStorage.getItem('codex-app-settings');
      if (stored) {
        ...
        localStorage.removeItem('codex-app-settings');
        return settings;
      }
    } catch {
    ...
    // No local data — seed defaults from ~/.codex/config.toml if available
    try {
      const codexConfig = await window.api.codexConfig.read();
      if (codexConfig && typeof codexConfig === 'object') {
        const envVars: ProviderEnvVar[] = [];
        if (typeof codexConfig.model === 'string' && codexConfig.model) {
          envVars.push({ key: 'CODEX_MODEL', value: codexConfig.model, enabled: true });
        }
        if (typeof codexConfig.model_reasoning_effort === 'string' && codexConfig.model_reasoning_effort) {
          envVars.push({ key: 'CODEX_REASONING_EFFORT', value: codexConfig.model_reasoning_effort, enabled: true });
        }
      ...
      // Ignore Codex config read errors
    }
  } catch {
...
interface SettingsStore {
  ...
  updateActiveProfile: (updates: Partial<CodexProfile>) => void;
