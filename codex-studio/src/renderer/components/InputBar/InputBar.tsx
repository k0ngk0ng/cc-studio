...
import type { AppearanceSettings, PermissionMode } from '../../types';
...
export function InputBar({ onSend, isStreaming, onStop }: InputBarProps) {
  ...
  const defaultMode = useSettingsStore(s => s.settings.general.autoApprove);
  ...
  const modeOptions: { value: PermissionMode; label: string; description: string }[] = [
    { value: 'readOnly', label: t('permissions.plan'), description: '--sandbox read-only' },
    { value: 'workspaceWrite', label: t('permissions.acceptEdits'), description: '--sandbox workspace-write' },
    { value: 'fullAuto', label: t('permissions.dontAsk'), description: '--full-auto' },
    { value: 'dangerFullAccess', label: t('permissions.bypassPermissions'), description: '--sandbox danger-full-access' },
  ];
  const [value, setValue] = useState('');
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  const [mode, setMode] = useState<PermissionMode>(defaultMode || 'workspaceWrite');
  ...
  const currentMode = modeOptions.find((m) => m.value === mode) ?? modeOptions[1];
      ...
      <div className={LAYOUT_CLASS[chatLayout] || LAYOUT_CLASS['centered']}>
        <div className="relative flex flex-col bg-surface rounded-xl border border-border focus-within:border-border-light transition-colors">
          ...
          <div
            ...
            className="absolute top-0 left-0 right-0 h-2 -translate-y-1/2 cursor-row-resize z-10"
          />
          {/* @ File search popup */}
          <FileSearchPopup
                  ...
                  {modeOptions.map((opt) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (processId) {
                          window.api.codex.setPermissionMode(processId, opt.value);
                        }
                      }}
