...
export function deriveSession(ecdh: crypto.ECDH, peerPublicKeyHex: string, pairingCode: string): E2EESession {
  ...
  const info = Buffer.from('cc-studio-e2ee', 'utf-8');
 ...
 * 1. CC_STUDIO_INSTANCE_ID env var - use custom instance ID (useful for dev)
...
export function generateDesktopId(): string {
  ...
  const instanceId =
    process.env.CC_STUDIO_INSTANCE_ID
    || process.env.CODEX_STUDIO_INSTANCE_ID;
  if (instanceId) {
    return crypto.createHash('sha256').update(instanceId).digest('hex').substring(0, 16);
  }

  const configDir = path.join(os.homedir(), '.codex-studio');
