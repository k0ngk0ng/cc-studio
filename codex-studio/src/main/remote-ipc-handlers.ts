...
function getServerUrl(): string {
  try {
    const settingsPath = path.join(os.homedir(), '.codex-studio', 'settings.json');
...
export function registerRemoteIpcHandlers(): void {
  ...
  // Initialize Codex message forwarding to mobile devices
  initCodexMessageForwarding();
...
// Track which Codex processId belongs to which mobile device
// so we can forward streaming messages back via relay.
const processToDevice = new Map<string, string>();

/**
 * Extract text content from a Codex CLI message for mobile display.
 */
function extractMessageContent(message: any): { role: string; content: string } | null {
  if (message?.type === 'item.completed' && message.item?.type === 'agent_message' && message.item?.text) {
    return {
      role: 'assistant',
      content: String(message.item.text),
    };
  }
...
/**
 * Initialize forwarding of Codex streaming messages to mobile devices.
 * Called once during handler registration.
 */
function initCodexMessageForwarding(): void {
  // Listen for all Codex messages and forward to the appropriate mobile device
  codexProcessManager.on('message', (pid: string, message: unknown) => {
    ...
    if (msg.type === 'thread.started' && typeof msg.thread_id === 'string') {
      relayClient.sendEvent(mobileDeviceId, 'studio:thread-started', {
        threadId: msg.thread_id,
      });
    }

    // Forward assistant text messages
    if (msg.type === 'assistant' || (msg.type === 'item.completed' && msg.item?.type === 'agent_message')) {
      const extracted = extractMessageContent(msg);
      if (extracted) {
        relayClient.sendEvent(mobileDeviceId, 'studio:message', {
    ...
    // Forward result / turn completion
    if (msg.type === 'turn.completed') {
      relayClient.sendEvent(mobileDeviceId, 'studio:stream-end', {});
    }
  ...
  codexProcessManager.on('exit', (pid: string) => {
    const mobileDeviceId = processToDevice.get(pid);
    if (mobileDeviceId) {
      relayClient.sendEvent(mobileDeviceId, 'studio:stream-end', {});
    }
...
async function handleRemoteCommand(fromDeviceId: string, parsed: unknown): Promise<void> {
  ...
  const channelAliases: Record<string, string> = {
    'studio:spawn': 'codex:spawn',
    'studio:send': 'codex:send',
    'studio:kill': 'codex:kill',
    'studio:permission-response': 'codex:permission-response',
    'studio:setPermissionMode': 'codex:setPermissionMode',
  };
  const internalChannel = channelAliases[msg.channel] || msg.channel;

  // Whitelist of allowed remote channels (exclude dangerous local-only operations)
  const ALLOWED_CHANNELS = new Set([
    // Shared studio remote protocol
    'studio:spawn', 'studio:send', 'studio:kill',
    'studio:permission-response', 'studio:setPermissionMode',
    // Codex
    'codex:spawn', 'codex:send', 'codex:kill',
    'codex:permission-response', 'codex:setPermissionMode',
    // Sessions
    ...
    'app:getModel', 'app:getCodexCliVersion',
    // Settings (read-only)
    'settings:read',
  ...
  try {
    const result = await invokeHandler(internalChannel, msg.args || []);

    // Track processId → mobile device mapping for remote-spawned sessions
    if (internalChannel === 'codex:spawn' && typeof result === 'string') {
      processToDevice.set(result, fromDeviceId);
    }

    // Clean up mapping when a remote kill is called
    if (internalChannel === 'codex:kill' && msg.args?.[0]) {
