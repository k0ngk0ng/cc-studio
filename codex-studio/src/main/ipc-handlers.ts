...
import { getPlatform, getCodexBinary, getCodexHome, getCodexModel, checkDependencies, readCodexConfig, writeCodexConfig } from './platform';
...
const CDN_BASE_URL = process.env.ALIYUN_OSS_CDN_URL || process.env.CODEX_STUDIO_CDN_URL || process.env.CLAUDE_STUDIO_CDN_URL || '';

// GitHub repository info for updates
const GITHUB_OWNER = 'k0ngk0ng';
const GITHUB_REPO = 'codex-studio';
...
export function registerIpcHandlers(): void {
  ...
  codexProcessManager.on('message', (pid: string, message: unknown) => {
    const wc = getWebContents();
    if (wc) {
      wc.send('codex:message', pid, message);
    }
  });

  codexProcessManager.on('exit', (pid: string, code: number, signal: string) => {
    const wc = getWebContents();
    if (wc) {
      wc.send('codex:message', pid, {
        type: 'exit',
        code,
        signal,
      });
  ...
  codexProcessManager.on('error', (pid: string, errorMsg: string) => {
    const wc = getWebContents();
    if (wc) {
      wc.send('codex:message', pid, {
        type: 'error',
  ...
  codexProcessManager.on('permission-request', (pid: string, request: unknown) => {
    const wc = getWebContents();
    if (wc) {
      wc.send('codex:permission-request', pid, request);
    }
  });

  handle('codex:spawn', async (_event, cwd: string, sessionId?: string, permissionMode?: string, envVars?: Array<{ key: string; value: string; enabled: boolean }>, language?: string, mcpServers?: Array<{ id: string; name: string; command: string; args: string[]; env: Record<string, string>; enabled: boolean }>, includeCoAuthoredBy?: boolean) => {
    ...
    return codexProcessManager.spawn(cwd, sessionId, permissionMode, envVars, language, mcpServers, includeCoAuthoredBy);
  });

  handle('codex:send', (_event, processId: string, content: string) => {
    return codexProcessManager.sendMessage(processId, content);
  });

  handle('codex:kill', (_event, processId: string) => {
    return codexProcessManager.kill(processId);
  });
  ...
  handle('codex:permission-response', (_event, processId: string, requestId: string, response: { behavior: 'allow' | 'deny'; updatedInput?: Record<string, unknown>; message?: string }) => {
    return codexProcessManager.respondToPermission(processId, requestId, response);
  });

  // Runtime permission mode change
  handle('codex:setPermissionMode', async (_event, processId: string, mode: string) => {
    return codexProcessManager.setPermissionMode(processId, mode);
  });
  ...
  handle('app:getCodexCliVersion', () => {
    // Keep the legacy IPC name but return the Codex CLI version.
    try {
      const codexPath = getCodexBinary();
      const raw = execSync(`${codexPath} --version`, {
        encoding: 'utf-8',
        timeout: 5000,
        stdio: ['pipe', 'pipe', 'pipe'],
      }).trim();
  ...
  handle('app:installCodexCli', async () => {
    const wc = getWebContents();
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const child = execFile(npmCmd, ['install', '-g', '@openai/codex'], {
          timeout: 120000,
          env: { ...process.env },
  ...
  handle('app:checkForUpdates', async () => {
    ...
    if (CDN_BASE_URL) {
      try {
        ...
        }>(`${CDN_BASE_URL}/codex-studio/releases/latest/release-info.json`);
        ...
        if (cdnData.version) {
          ...
          return {
            ...
            tagName,
            name: `CodexStudio ${cdnData.version}`,
            body: '',
    ...
    try {
      ...
      if (CDN_BASE_URL) {
        try {
          const cdnData = await fetchJson<{ files: Record<string, string> }>(`${CDN_BASE_URL}/codex-studio/releases/${tagName}/release-info.json`);
  ...
  handle('app:downloadUpdate', async (_event, platform: string) => {
    ...
    try {
      ...
      // Download the file
      const downloadDir = path.join(app.getPath('temp'), 'codex-studio-updater');
    ...
    if (pendingPath && fs.existsSync(pendingPath)) {
        ...
        try {
          ...
          if (mountMatch) {
            ...
            if (apps.length > 0) {
              ...
              const targetApp = '/Applications/CodexStudio.app';
      ...
      if (pendingPath.endsWith('.exe')) {
        try {
          ...
          const batchPath = path.join(app.getPath('temp'), 'codex-studio-update.cmd');
          const batchContent = [
            '@echo off',
    ...
    try {
      ...
      if (isPackaged) {
        ...
        if (!fs.existsSync(pkgJsonPath)) {
          fs.writeFileSync(pkgJsonPath, JSON.stringify({ name: 'codex-studio-runtime', private: true }, null, 2));
        }
  ...
  handle('codexConfig:read', () => {
    return readCodexConfig();
  });

  handle('codexConfig:write', (_event, updates: Record<string, unknown>) => {
    writeCodexConfig(updates);
    return true;
  });

  // ─── App Settings (~/.codex-studio/settings.json) ──────────────────
  const settingsDir = path.join(os.homedir(), '.codex-studio');
  ...
  // ─── Skills (~/.codex/skills/) ─────────────────────────────────────
  const globalSkillsDir = path.join(getCodexHome(), 'skills');
  ...
  handle('skills:list', async () => {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        ...
        dirPath,
        filePath: skillFile,
        hasTemplate: fs.existsSync(path.join(dirPath, 'CODEX.md.template')),
  ...
  const globalCommandsDir = path.join(getCodexHome(), 'commands');
