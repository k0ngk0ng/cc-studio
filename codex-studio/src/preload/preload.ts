...
const codexMessageListeners = new Map<Function, (...args: any[]) => void>();
const codexPermissionListeners = new Map<Function, (...args: any[]) => void>();
...
contextBridge.exposeInMainWorld('api', {
  codex: {
    spawn: (cwd: string, sessionId?: string, permissionMode?: string, envVars?: Array<{ key: string; value: string; enabled: boolean }>, language?: string, mcpServers?: McpServer[], includeCoAuthoredBy?: boolean) =>
      ipcRenderer.invoke('codex:spawn', cwd, sessionId, permissionMode, envVars, language, mcpServers, includeCoAuthoredBy),
    send: (processId: string, content: string) =>
      ipcRenderer.invoke('codex:send', processId, content),
    kill: (processId: string) => ipcRenderer.invoke('codex:kill', processId),
    onMessage: (callback: (processId: string, message: unknown) => void) => {
      ...
      codexMessageListeners.set(callback, wrappedCallback);
      ipcRenderer.on('codex:message', wrappedCallback);
    },
    removeMessageListener: (
      callback: (processId: string, message: unknown) => void
    ) => {
      const wrappedCallback = codexMessageListeners.get(callback);
      if (wrappedCallback) {
        ipcRenderer.removeListener('codex:message', wrappedCallback);
        codexMessageListeners.delete(callback);
      }
    },
    onPermissionRequest: (callback: (processId: string, request: unknown) => void) => {
      ...
      codexPermissionListeners.set(callback, wrappedCallback);
      ipcRenderer.on('codex:permission-request', wrappedCallback);
    },
    removePermissionRequestListener: (callback: (processId: string, request: unknown) => void) => {
      const wrappedCallback = codexPermissionListeners.get(callback);
      if (wrappedCallback) {
        ipcRenderer.removeListener('codex:permission-request', wrappedCallback);
        codexPermissionListeners.delete(callback);
      }
    },
    respondToPermission: (
      processId: string,
      requestId: string,
      response: { behavior: 'allow' | 'deny'; updatedInput?: Record<string, unknown>; message?: string }
    ) => ipcRenderer.invoke('codex:permission-response', processId, requestId, response),
    setPermissionMode: (processId: string, mode: string) =>
      ipcRenderer.invoke('codex:setPermissionMode', processId, mode),
  } satisfies CodexAPI,
  ...
  app: {
    ...
    getCodexCliVersion: () => ipcRenderer.invoke('app:getCodexCliVersion'),
    getGitVersion: () => ipcRenderer.invoke('app:getGitVersion'),
    getNodeVersion: () => ipcRenderer.invoke('app:getNodeVersion'),
    installCodexCli: () => ipcRenderer.invoke('app:installCodexCli'),
  ...
  codexConfig: {
    read: () => ipcRenderer.invoke('codexConfig:read'),
    write: (updates: Record<string, unknown>) => ipcRenderer.invoke('codexConfig:write', updates),
  } satisfies CodexConfigAPI,
